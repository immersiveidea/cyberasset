    ...
    const rowRender =
        docs.map((row, index) => {
            if (selectedComponent == row._id) {
                return <Button disabled fullWidth size="compact-md" id={row._id} key={row._id} variant="light"
                               onClick={selectComponent}>{row.name || row._id}</Button>
            } else {
                return <Button fullWidth size="compact-md" id={row._id} key={row._id} variant="light"
                               onClick={selectComponent}>{row.name || row._id}</Button>
            }
